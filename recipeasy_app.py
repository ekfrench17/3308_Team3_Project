...
def community():
    ...
    if request.method == "POST":
        ...
        try: 
            ...
            message = "rating conversion failed"
        try:
            success,message = create_post('RecipEASYDB', result['postContent'], result['recipe_name'], 'elaine17', rating) #update with session id later
            #output = render_template("community.html")
            if success == True: 
                output = render_template("community.html", all_recipes=all_recipes)
            else:
